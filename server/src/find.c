...
void cmd_incantation(server_t *server, player_t *p)
{
    ...
    if (p->lvl < 1 || p->lvl > 7) {
        dprintf(p->fd, "ko\n");
        printf("Invalid level for incantation: %d\n", p->lvl);
        return;
    }
    req = get_elevation_requirements(p->lvl);
    dprintf(p->fd, "Elevation underway\n");
    if (!validate_incantation_requirements(server, p, req)) {
        dprintf(p->fd, "ko\n");
        return;
    }
    consume_incantation_resources(server, p, req);
    elevate_all_participants(server, p);
    if (server->gui_fd > 0) {
        dprintf(server->gui_fd, "pic %d %d %d %d\n", 
               p->x, p->y, p->lvl, p->id);
    }
    printf("Successful incantation: %d players elevated to level %d\n",
